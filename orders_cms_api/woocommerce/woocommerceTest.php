...
// class to connect with the woocommerce api
class woocommerceApiClass
{
    ...
    function __construct()
    {
        ...
        $this->url = 'http://localhost/wp-woo-commerce/wp-json/wc/v3/';
        ...
        // $this->marketplace = 'woocommerce';
    }

    public function resquest_woocommerce_api()
    {
    ...
    {
        // get oauth according with the parameters
        $params = array(
            'oauth_consumer_key' => 'ck_2b58d19c44085a335db8e129c8a83fdf6f1dfbcc',
            'oauth_signature_method' => 'HMAC-SHA1',
            'oauth_timestamp' => '1624446133',
            'oauth_nonce' => '4kTt1bNhtrU',
            'oauth_signature' => 'm/Vwvv5MAFk3vxkRpNbY/JQyk9w=',
        );
        $this->resource = 'orders.json';
        $this->parameter = http_build_query($params);
        $oauth = $this->resquest_woocommerce_api();

        $oauth = json_decode($oauth, true);

        return json_decode($oauth, true);
    }
    ...
    public function orders()
    {
        ...
        $this->resource = 'orders.json';
        $this->parameter = http_build_query($params);
        $orders = $this->resquest_woocommerce_api();
...
try {
    ...
    $oauth = $woocommerceApi->oauth();
    if (empty($oauth)) {
        throw new reportException('No oauth found');
    }
    $orders = $woocommerceApi->orders();
    if (empty($orders)) {
        throw new reportException('No orders found');
    }

    // execute the method to merge all data and create a row for each product 
    $data = $woocommerceApi->merge_and_organize_data($orders);
    
    // execute the method export to cvs 
    # $woocommerceApi->export_as_cvs($data);
