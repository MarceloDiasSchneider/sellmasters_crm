...
class prestashopApiClass
{
    ...
    {
        foreach ($orders['orders'] as $key => $order) {
            // remove all products with STUDIO00 "product_reference"
            // create a STUDIO00_discount to decrease of "total_discounts_tax_incl"
            foreach ($order['associations']['order_rows'] as $key => $product) {
                if ($product['product_reference'] == 'STUDIO00') {
                    $order['total_discounts_tax_incl'] -= $product['unit_price_tax_incl'];
                    unset($order['associations']['order_rows'][$key]);
                }
            }
            // create the id_order to not overwrite on merge
            $order['id_order'] = $order['id'];
            // removing the hour from the date
            $format_date = new DateTime($order['date_add']);
            $order['date_add_formated'] = $format_date->format('Y-m-d');
            // set the data defined in __construc 
            $order['merchant_id'] = $this->merchant_id;
            $order['marketplace'] = $this->marketplace;
            $order['order_rows'] = count($order['associations']['order_rows']);
            ...
            foreach ($order['associations']['order_rows'] as $key => $product) {
                ...
                // retrieve a brand from the product name
                $brand = explode('-', $product['product_name'], 2);
                $order['brand_from_product_name'] = $brand[0]; 
        ...
        $rows = array_map(function($data) {
            return array(
                ...
                // 'data' => $data['date_add_formated'],
                // 'ordine_marketplace' => $data['reference'],
                // 'nome_cliente' => $data['lastname'] . " " . $data['firstname'],
                // 'item_tax' => number_format($data['unit_price_tax_incl'], 2, ',', '') ,
                // 'shipping_tax' => number_format($data['total_shipping_tax_incl'], 2, ',', ''),
                // 'item_promotion_discount' => number_format($data['total_discounts_tax_incl'], 2, ',', ''),
                // 'totale' => number_format($data['unit_price_tax_incl'] + (
                //     $data['total_shipping_tax_incl'] / $data['order_rows']
                // ) - ( 
                //     $data['total_discounts_tax_incl'] / $data['order_rows']
                // ), 2, ',', ''),
                // 'sku' => $data['product_reference'],
                // 'brand' => $data['brand_from_product_name'],
                // 'product_name' => $data['product_name'],
                // 'prezzo_originale' => 'prendere da lengow',
                // 'sconto' => '',
    ...
    public function export_as_cvs($products)
    {
        ...
        $fp = fopen('orders_studioarch.csv', 'w');
